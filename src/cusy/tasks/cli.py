...
def add(summary: list[str], owner: str = typer.Option(None, "-o", "--owner")):
    ...
    with tasks_db() as db:
        db.add_task(tasks.Item(summary, owner, state="todo"))
...
def delete(task_id: int):
    ...
    with tasks_db() as db:
        ...
        except tasks.InvalidItemIdError:
...
def update(
    ...
    with tasks_db() as db:
        try:
            db.update_task(task_id, tasks.Item(summary, owner, state=None))
        except tasks.InvalidItemIdError:
            print(f"Error: Invalid task id {task_id}.")
